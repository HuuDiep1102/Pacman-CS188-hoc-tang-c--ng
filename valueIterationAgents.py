...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        "*** YOUR CODE HERE ***"
        # Vong lap gia tri duoc thiet lap cho cac lan lap
        for i in range(iterations):
          #khai bao 1 bo dem de luu gia tri cua lan lap cho moi trang thai
          iterationValues = util.Counter()
          #nhin vao tung state
          for s in self.mdp.getStates():
            # neu trang thai la trang thai ket thuc, phan thuong la phan thuong thoat va ko co phan thuong chiet khau
            if self.mdp.isTerminal(s):
              self.values[s] = self.mdp.getReward(s, 'exit', '')
            # neu chua phai trang thai ket thuc thi tim gia tri tot nhat lam tong phan thuonh
            else:
